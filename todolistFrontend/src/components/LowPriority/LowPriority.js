...
function LowPriority() {
  ...
  return (
    <div className='LowPriority border p-3'>
      {/*Heading */}
      <h1 className='text-dark text-center'>Low Priority Tasks</h1>
      {/*Add Task from */}
      ...
      {/*Incomplete Block */}
      <Button className='fs-4 btn-secondary mt-2 bg-light border-0 text-dark ' onClick={() => setOpenincomplete(!openincomplete)}> {openincomplete ? <AiOutlineDown /> : <AiOutlineRight />} Incomplete  </Button>
      <Collapse in={openincomplete}>
        <div className='incompleteblock'>
          {
            AllTask.map((task) => {
              if (task.Priority === 'Low' && task.Completestatus === 0) {
                return (
      ...
      <Collapse in={open}>
        <div className='completeblock'>
          {
            AllTask.map((task) => {
              if (task.Priority === 'Low' && task.Completestatus === 1) {
                return (
                  <div class="d-flex border border-secondary p-2 m-2 rounded taskbox" key={task.id}>
